...
describe('Create Order', () => {  
  it('Should return  400 if no orderNumber provided', async () => {    
    ...
    // @ts-ignore
    const { statusCode, body } = await createOrderUseCase.execute({ table: faker.random.number(),
  ...
  it('Should return  400 if no table  is provided', async () => {    
    ...
    // @ts-ignore
    const { statusCode, body } = await createOrderUseCase.execute({description: faker.random.words(),
  ...
  it('Should return 201 if the required data has been validated', async () => {
    ...
    const { statusCode } = await createOrderUseCase.execute({ table: faker.random.number(),
      description: faker.random.words(),
      orderNumber: faker.random.number()})
